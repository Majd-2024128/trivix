// Single canonical wallpaper list. Each wallpaper has a "dark" gradient (the original)
// and a "light" gradient derived by lightening every hex stop. This way the gallery
// stays identical between modes, and switching theme auto-swaps to the matching variant.
...
// Lighten a hex color: push lightness toward ~0.82 and slightly desaturate.
function lightenHex(hex) {
  const [h, s, l] = hexToHsl(hex);
  const newL = l + (0.85 - l) * 0.7; // pulls darks up strongly, leaves already-light alone
...
// Pre-render the lighter variants once.
export const WALLPAPERS = DARK_WALLPAPERS.map((w) => ({
  ...w,
  light: lightenGradient(w.dark),
}));

export const DEFAULT_WALLPAPER_ID = "green";
